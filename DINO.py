...
#
# NOTA SOBRE MULTIPROCESSING:
# Los niveles 4, 5 y 6 usan multiprocessing.Process (ver
# spawn_multiproceso.py). En Windows y macOS, multiprocessing
# necesita que todo el codigo "ejecutable" del archivo principal
# quede dentro de un bloque `if __name__ == "__main__":`; si no,
# cada proceso hijo volveria a ejecutar este archivo desde cero
# (abriendo ventanas nuevas sin fin). Por eso todo el contenido de
# este archivo (que antes estaba suelto) ahora vive dentro de
# main(), y main() solo se llama dentro del guard de abajo.
# La logica del juego es exactamente la misma, solo cambio la
# indentacion para cumplir este requisito de Python.
# ============================================================
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if estado == "MENU":
                    ...
                    if destino == "REANUDAR":
                        ...
                        # Los niveles 4, 5 y 6 usan un proceso secundario
                        # (multiprocessing) que normalmente se detiene solo
                        # al ganar o perder. Si el jugador sale desde la
                        # pausa sin terminar el nivel, se detiene aqui para
                        # no dejarlo corriendo de fondo. hasattr() protege
                        # a los niveles que no definen este metodo.
                        if nivel_actual is not None and hasattr(nivel_actual, "detener_procesos"):
                            nivel_actual.detener_procesos()
                        estado = "SELECTOR_NIVELES"
